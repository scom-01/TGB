...
public class ResultPanelUI : MonoBehaviour
{
    ...
    public void UpdateResultPanel(StatsData stats)
    {
        if (PhysicsAgg_Txt != null)
            PhysicsAgg_Txt.text = string.Format($"{stats.PhysicsAggressivePer} %");
        if (MagicAgg_Txt != null)
            MagicAgg_Txt.text = string.Format($"{stats.MagicAggressivePer} %");
        ...
        if (ElementalAgg_Txt != null)
            ElementalAgg_Txt.text = string.Format($"{stats.ElementalAggressivePer} %");
        if (ElementalDef_Txt != null)
            ElementalDef_Txt.text = string.Format($"{stats.ElementalDefensivePer} %");
        if (AttackSpeed_Txt != null)
            AttackSpeed_Txt.text = string.Format($"{stats.AttackSpeedPer} %");
